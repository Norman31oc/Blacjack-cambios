...
# 
# Corrección en calcular_valord:el valor del As no se está manejando correctamente cuando hay múltiples Ases. Deberías ajustar el valor del As si el total excede 21.
    def calcular_valor(self) -> Union[int, str]:
        ...
        for carta in self.cartas:
            valor += carta.calcular_valor(as_como_11=(valor + 11 <= 21))
            if carta.valor == "A":
                numero_de_ases += 1

        while valor > 21 and numero_de_ases > 0:
            valor -= 10
            numero_de_ases -= 1
...
# 
# Verificación de que el jugador tiene suficientes fichas antes de apostar.

    def verificar_finchas(self, apuesta: int):
        if not self.jugador.puede_apostar(apuesta):
            print (ValueError("No tienes suficientes fichas para hacer esta apuesta."))
        self.apuesta_actual = apuesta
    ...
    def hay_empate(self) -> bool:
        valor_mano_jugador = self.jugador.mano.calcular_valor()
        valor_mano_casa = self.cupier.mano.calcular_valor()
        return valor_mano_casa == valor_mano_jugador and not self.jugador.mano.es_blackjack() and not self.cupier.mano.es_blackjack()
    
    # 
    # Gestionar la apuesta y las fichas del jugador al final del juegO.
    def finalizar_juego(self):
        if self.jugador_gano():
            self.jugador.agregar_fichas(self.apuesta_actual * 2)
        elif not self.hay_empate():
